            ...
            <td class="title" style="vertical-align: top" valign="top">
                ...
                <div class="btn-group dropright">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" style="">
                        @foreach(\AmplifyCode\AscentCMS\Models\BlockTemplate::orderBy('name')->get() as $template)
                        <a class="dropdown-item text-sm btn-delete" href="{{ action([AmplifyCode\AscentCMS\Controllers\Admin\BlockController::class, 'create'], ['stack_id' => $item->id, 'blocktemplate_id'=>$template->id]) }}">
                        <b>{{ $template->name }}</b>
                        <br/>
                ...
                {{-- <div class="pt-1"><a class="btn btn-secondary btn-sm" href="{{ action([AmplifyCode\AscentCMS\Controllers\Admin\BlockController::class, 'create'], ['stack_id' => $item->id, 'blocktemplate_id'=>1]) }}">New Block</a></div> --}}
            </td>
                ...
                <table class="menuitems" id="stackblocks">
                    @foreach($item->blocks as $block)
                        <tr class="{{ $block->published ? 'published' : 'hidden' }}">
                            <td>
                                <A href="{{ action([AmplifyCode\AscentCMS\Controllers\Admin\BlockController::class, 'edit'], ['block' => $block->id]) }}">{{ $block->name }}</A>
                                <input name="blockorder[]" type="hidden" value="{{ $block->id }}" /></td>
                            </td>
                            <td>{{ $block->template->name }}</td>
                            <td><A class="modal-link" href="{{ action([AmplifyCode\AscentCMS\Controllers\Admin\BlockController::class, 'delete'], ['block' => $block->id]) }}">[x]</A></td>
                        </tr>
                    @endforeach
